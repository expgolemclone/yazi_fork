...
function Rails:build()
	...
	if c.position == "top" then
		if c.bottom.h > 0 and c[3].h > 0 then
			children[#children + 1] = Rail:new("rail-top", c.bottom { h = math.min(1, c.bottom.h) }, c)
		end
		if c[1].w > 0 and c[2].w > 0 then
